...
export const InductionForm = () => {
  ...
  const validateSection = (section: number): boolean => {
    switch (section) {
      case 0:
        return !!(formData.fullName && formData.bitsId && formData.mobileNumber && formData.email);
      case 1:
  ...
  const renderSection = () => {
    switch (currentSection) {
      case 0:
        return <PersonalInfoSection data={formData} updateData={updateFormData} />;
      case 1:
          ...
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-6 flex items-center gap-2">
              {currentSection < 3 && (
                <CheckCircle2 
                  className={`w-6 h-6 ${
                    validateSection(currentSection) ? 'text-primary' : 'text-muted-foreground'
                  }`} 
                />
              )}
